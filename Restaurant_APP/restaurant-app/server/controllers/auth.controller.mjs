...
const login = async (req, res, next) => {
  try {
    ...
    if (user.deletedAt !== null) return res.status(403).json({ error: 'Acest cont a fost dezactivat!' })
...
const logout = async (req, res, next) => {
  try {
    ...
    if (token) {
      ...
      } catch { /* token invalid, continuam oricum */ }
    }
...
const verificaEmail = async (req, res, next) => {
  try {
    ...
    const user = await db.User.findOne({ where: { email } })
    if (!user) return res.status(404).json({ error: 'Nu exista niciun cont cu acest email!' })
    if (user.deletedAt !== null) return res.status(403).json({ error: 'Acest cont a fost dezactivat.' })
...
const resetParola = async (req, res, next) => {
  try {
    ...
    if (!user) return res.status(404).json({ error: 'Nu exista niciun cont cu acest email!' })
    if (user.deletedAt !== null) return res.status(403).json({ error: 'Acest cont a fost dezactivat.' })
    await user.update({ passwordHash: await bcrypt.hash(parolaNoua, 10) })
    res.json({ message: 'Parola a fost resetata cu succes!' })
  } catch (err) { next(err) }
}

export default { register, login, adminLogin, logout, verificaEmail, resetParola }
